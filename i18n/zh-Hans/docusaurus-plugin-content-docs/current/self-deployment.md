...
早期版本用一个全实例的 `TELEGRAM_BOT_TOKEN`。该配置项已移除，升级是自动的：

- 升级后首次启动时，旧 token 会迁移为实例所有者自己的 bot，其创建过的会话与投递记录都会打上该 bot 的标记，
  旧配置项随之删除。所有者无需任何操作即可继续使用。
- 若未设置 `UD_ENCRYPTION_KEY`，迁移会跳过并给出警告，旧配置保持不动——什么都不会丢失，
  但通道会一直停着，直到你设置密钥并重启；之后所有者到 **设置 → 即时通讯** 粘贴 token 即可。
